...
struct ZEBlobStore
{
    ...
    u8 *GetFreeSlot(i32 id)
    {
        u8 *mem = NULL;
        i32 index = -1;
        zErrorCode err = m_array->GetFreeSlot(&mem, &index, id);
        ZE_ASSERT(err == ZERROR_CODE_NONE, "Could not get free blob slot\n");
        ZEHashTableData data;
        data.i = index;
        err = m_lookup->Insert(id, data);
        ZE_ASSERT(err == ZERROR_CODE_NONE, "Could not insert lookup key\n");
        return mem;
    }

    zErrorCode MarkForRemoval(i32 id)
    {
        // Lookup key
        i32 index = m_lookup->FindI32(id, ZE_LT_INVALID_INDEX);
        ZE_ASSERT(index != ZE_LT_INVALID_INDEX, "Blob store could not find lookup key\n");
        // Blob
        zErrorCode err = m_array->MarkForFree(index);
        ZE_ASSERT(err == ZERROR_CODE_NONE, "Error marking blob for removal\n");
        return err;
    }
    ...
    i32 Truncate()
    {
        ...
        while (i >= 0)
        {
            ...
            if (i == lastIndex)
            {
                ...
                ZE_ASSERT(err == ZERROR_CODE_NONE, "Blob store - Error removing lookup key")
...
static zErrorCode ZE_InitBlobStore(
