...
struct ZEBlobArray
{
     ...
     */
    zErrorCode GetFreeSlot(u8 **dataResult, i32 *indexResult, i32 id)
    {
        if (id == m_invalidId)
        {
            return ZERROR_CODE_BAD_PARAMETER;
        }
    ...
    zErrorCode MarkForFree(i32 index)
    {
        if (index >= m_numBlobs)
        {
            return ZERROR_CODE_OUT_OF_BOUNDS;
        }
        ZEBlobHeader *header = GetHeaderByIndex(index);
        if (header == NULL)
        {
            return ZERROR_CODE_NOT_FOUND;
        }
...
static zErrorCode ZE_CreateBlobArray(
    ...
    ZEBlobArray **result, i32 capacity, zeSize sizePerObject, i32 invalidId)
{
    ...
    if (mem == NULL)
    {
        return ZERROR_CODE_ALLOCATION_FAILED;
    }
